/*
 * lckpwdf.c -- prevent simultaneous updates of password files
 *
 * Before modifying any of the password files, call lckpwdf().  It may block
 * for up to 15 seconds trying to get the lock.  Return value is 0 on success
 * or -1 on failure.  When you are done, call ulckpwdf() to release the lock.
 * The lock is also released automatically when the process exits.  Only one
 * process at a time may hold the lock.
 *
 * These functions are supposed to be conformant with AT&T SVID Issue 3.
 *
 * Written by Marek Michalkiewicz <marekm@i17linuxb.ists.pwr.wroc.pl>,
 * public domain.
 *
 * UPDATE:
 * Terry Ron Vantreese 1 August 2001
 * Expanded to lock any file with any timeouts allowing 
 * multiple read-only access or one write access.
 */

#include <string.h>
#include <esconfig.h>

#if defined(sparc_Solaris_spro) || defined(ix86_Unix_gcc2) || defined(sparc_Unix_gcc2)

#include <fcntl.h>
#include <signal.h>
#include <unistd.h>
#include <sys/stat.h>

static int set_close_on_exec(int fd)
{
    int flags = fcntl(fd, F_GETFD, 0);
    if (flags == -1)
        return -1;
    flags |= FD_CLOEXEC;
    return fcntl(fd, F_SETFD, flags);
}

static int do_lock(int fd, int lockm)
{
    struct flock fl;
    
    memset(&fl, 0, sizeof fl);
    fl.l_whence = SEEK_SET;
    switch (lockm)
    {
    case 1:
        fl.l_type = F_WRLCK;
        break;
    case 0:
        fl.l_type = F_RDLCK;
        break;
    }
    return fcntl(fd, F_SETLKW, &fl);
}

static void alarm_catch(int sig)
{
/* 
    does nothing, but fcntl F_SETLKW will fail with EINTR 
*/
}

int lockfile(char *lockfilename, char *mode, unsigned int timeout)
{
    int lockfd, lockm;
    struct sigaction act, oldact;
    sigset_t set, oldset;

    if (strchr(mode, 'a') || strchr(mode, 'w') || strchr(mode, '+'))
    {
        lockfd = open(lockfilename, O_CREAT | O_WRONLY, (S_IREAD | S_IWRITE));
        lockm = 1;
    }
    else
    {
        lockfd = open(lockfilename, O_CREAT | O_RDWR, (S_IREAD | S_IWRITE));
        lockm = 0;
    }
    if (lockfd == -1)
        return -1;
    if (set_close_on_exec(lockfd) == -1)
        goto cleanup_fd;

    memset(&act, 0, sizeof act);
    act.sa_handler = alarm_catch;
    act.sa_flags = 0;
    sigfillset(&act.sa_mask);
    if (sigaction(SIGALRM, &act, &oldact) == -1)
        goto cleanup_fd;

    sigemptyset(&set);
    sigaddset(&set, SIGALRM);
    if (sigprocmask(SIG_UNBLOCK, &set, &oldset) == -1)
        goto cleanup_sig;

    alarm(timeout);
    if (do_lock(lockfd, lockm) == -1)
        goto cleanup_alarm;
    alarm(0);
    sigprocmask(SIG_SETMASK, &oldset, NULL);
    sigaction(SIGALRM, &oldact, NULL);
    return lockfd;

cleanup_alarm:
    alarm(0);
    sigprocmask(SIG_SETMASK, &oldset, NULL);
cleanup_sig:
    sigaction(SIGALRM, &oldact, NULL);
cleanup_fd:
    close(lockfd);
    lockfd = -1;
    return -1;
}

int ulockfile(int lockfd)
{
    if (lockfd == -1)
        return -1;

    if (close(lockfd) == -1)
    {
        return -1;
    }
    return 0;
}

#elif defined(ix86_MSWin_msvc)

/*
    Not available in Microsoft :(
*/
int lockfile(char *lockfilename, char *mode, unsigned int timeout)
{
    return -1;
}

int ulockfile(int lockfd)
{
    return -1;
}

#else

#error Unable to compile lockfile() in lockfile.c

#endif
